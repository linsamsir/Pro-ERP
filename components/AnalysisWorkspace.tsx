...
import { 
  PieChart, Truck, Package, HardHat, TrendingUp, Download, 
  Calendar, ArrowRight, Eye, EyeOff, Plus, Trash2, AlertCircle, RefreshCw
} from 'lucide-react';
...
const AnalysisWorkspace: React.FC = () => {
  ...
  const SidebarItem = ({ id, icon: Icon, label }: any) => (
    <button 
      ...
      className={`w-full flex items-center gap-3 px-4 py-4 rounded-xl font-black transition-all ${activeTab === id ? 'bg-[#78b833] text-white shadow-md' : 'text-[#b59a7a] hover:bg-[#fffdf5]'}`}
    >
      <Icon size={20} /> {label}
    </button>
  );
  ...
  return (
    <div className="flex flex-col lg:flex-row h-screen overflow-hidden bg-[#fbf8e6]">
      
      {/* Sidebar */}
      <aside className="w-full lg:w-64 bg-white border-r border-[#e8dcb9] flex flex-col p-6 shrink-0 overflow-y-auto">
        <div className="mb-8">
           <h1 className="text-h2 flex items-center gap-2"><PieChart className="text-orange-500"/> 進階分析</h1>
           <div className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-1">Analysis Workspace</div>
        </div>
        <nav className="space-y-2 flex-1">
           <SidebarItem id="dashboard" icon={TrendingUp} label="經營成效總覽" />
           <SidebarItem id="assets" icon={Truck} label="設備資產管理" />
           <SidebarItem id="stock" icon={Package} label="耗材庫存成本" />
        </nav>
        <div className="mt-4 pt-4 border-t border-slate-100 space-y-3">
           <div className="bg-orange-50 p-3 rounded-lg text-xs font-bold text-orange-700 leading-relaxed">
              <HardHat size={14} className="inline mr-1"/>
              人力成本設定請至<br/>「老闆戰情室 > 齒輪」
           </div>
           <button onClick={toggleDemo} className="w-full flex items-center gap-2 px-4 py-3 rounded-lg text-sm font-bold text-slate-400 hover:bg-slate-50 transition-colors">
              {isDemo ? <EyeOff size={16}/> : <Eye size={16}/>} {isDemo ? '關閉展示模式' : '開啟展示模式'}
           </button>
           <button onClick={exportCSV} className="w-full flex items-center gap-2 px-4 py-3 rounded-lg text-sm font-bold text-[#78b833] bg-[#f0fdf4] border border-[#dcfce7] hover:bg-[#dcfce7] transition-colors">
              <Download size={16}/> 匯出本月報表
           </button>
        </div>
      </aside>
      ...
      <main className="flex-1 overflow-y-auto p-4 lg:p-8">
        ...
        {/* Month Selector */}
        <div className="flex items-center justify-between mb-8 bg-white p-4 rounded-3xl border-2 border-[#e8dcb9] shadow-sm max-w-3xl">
           <button onClick={() => changeMonth(-1)} className="p-2 bg-slate-100 rounded-full hover:bg-slate-200"><ArrowRight className="rotate-180" size={20}/></button>
           <div className="text-xl font-black text-[#5d4a36] flex items-center gap-3">
              <Calendar size={24} className="text-[#b59a7a]"/>
              {currentDate.getFullYear()} 年 {currentDate.getMonth() + 1} 月
           </div>
           <button onClick={() => changeMonth(1)} className="p-2 bg-slate-100 rounded-full hover:bg-slate-200"><ArrowRight size={20}/></button>
        </div>
          ...
          <div className="space-y-8 animate-pop">
             <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="ac-card border-l-8 border-l-red-400">
                   ...
                   <div className="text-sm text-slate-400 font-bold mt-2">包含: 人力/耗材/折舊/油資</div>
                </div>
             ...
             <div className="bg-white rounded-[2rem] shadow-sm border-2 border-[#e8dcb9] overflow-hidden">
                <div className="p-6 border-b border-[#e8dcb9] bg-[#fffbf0]">
                   <h3 className="text-h3 text-[#5d4a36]">任務獲利能力分析 (L2-5)</h3>
                   <p className="text-note mt-1">此表為動態運算，不會回寫修改您的原始工單</p>
                </div>
                   ...
                   <table className="w-full text-left">
                      <thead className="bg-slate-50 text-xs font-black text-slate-400 uppercase tracking-wider">
                         <tr>
                            ...
                            <th className="p-5 text-right text-red-300">人力攤提</th>
        ...
        {activeTab === 'assets' && (
           <div className="space-y-6 animate-pop">
              <div className="bg-orange-50 p-4 rounded-2xl border border-orange-200 text-sm font-bold text-orange-800 flex items-center justify-between">
                 ...
                 <button onClick={() => { db.l2.assets.seed(); refresh(); }} className="bg-white border px-3 py-1 rounded-lg text-xs shadow-sm active:scale-95">
